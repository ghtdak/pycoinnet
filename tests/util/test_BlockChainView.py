...
def test_halfsies():
    ...
    assert hi(3) == {0, 1, 2, 3}
    assert hi(4) == {0, 2, 3, 4}
    assert hi(5) == {0, 2, 3, 4, 5}
    assert hi(6) == {0, 2, 4, 5, 6}
    assert hi(7) == {0, 2, 4, 5, 6, 7}
    assert hi(8) == {0, 2, 4, 6, 7, 8}
    assert hi(9) == {0, 2, 4, 6, 7, 8, 9}
    assert hi(10) == {0, 4, 6, 8, 9, 10}
    assert hi(14) == {0, 4, 8, 10, 12, 13, 14}
    assert hi(16) == {0, 4, 8, 10, 12, 14, 15, 16}
    assert hi(24) == {0, 8, 12, 16, 18, 20, 22, 23, 24}
    assert hi(32) == {0, 8, 16, 20, 24, 26, 28, 30, 31, 32}
    assert hi(33) == {0, 8, 16, 20, 24, 26, 28, 30, 31, 32, 33}
    assert hi(34) == {0, 8, 16, 20, 24, 28, 30, 32, 33, 34}
    assert hi(1000) == {0, 256, 384, 512, 640, 704, 768, 832, 864, 896, 928,
                        944, 960, 968, 976, 984, 988, 992, 994, 996, 998, 999,
                        1000}
    print(sorted(hi(150000)))
    assert hi(100000) == {0, 32768, 49152, 65536, 73728, 81920, 86016, 90112,
                          92160, 94208, 95232, 96256, 97280, 97792, 98304,
                          98816, 99072, 99328, 99456, 99584, 99712, 99776,
                          99840, 99872, 99904, 99936, 99952, 99968, 99976,
                          99984, 99988, 99992, 99994, 99996, 99998, 99999,
                          100000}
    assert hi(150000) == {0, 32768, 65536, 81920, 98304, 114688, 122880, 131072,
                          135168, 139264, 141312, 143360, 145408, 146432,
                          147456, 147968, 148480, 148736, 148992, 149248,
                          149376, 149504, 149632, 149696, 149760, 149824,
                          149856, 149888, 149920, 149936, 149952, 149968,
                          149976, 149984, 149988, 149992, 149994, 149996,
                          149998, 149999, 150000}
    assert hi(200000) == {0, 65536, 98304, 131072, 147456, 163840, 172032,
                          180224, 184320, 188416, 190464, 192512, 194560,
                          195584, 196608, 197632, 198144, 198656, 198912,
                          199168, 199424, 199552, 199680, 199744, 199808,
                          199872, 199904, 199936, 199952, 199968, 199976,
                          199984, 199988, 199992, 199994, 199996, 199998,
                          199999, 200000}
    assert hi(1000000) == {0, 262144, 393216, 524288, 655360, 720896, 786432,
                           851968, 884736, 917504, 933888, 950272, 966656,
                           974848, 983040, 987136, 991232, 993280, 995328,
                           996352, 997376, 997888, 998400, 998912, 999168,
                           999424, 999552, 999680, 999744, 999808, 999872,
                           999904, 999936, 999952, 999968, 999976, 999984,
                           999988, 999992, 999994, 999996, 999998, 999999,
                           1000000}
    prior = hi(0)
    for i in range(500):
        v = hi(i)
