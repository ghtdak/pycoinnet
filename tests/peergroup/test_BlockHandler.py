...
def make_add_peer(fast_forward_add_peer, blockfetcher, block_handler,
    ...
    def add_peer(peer, other_last_block_index):
        ...
        blockfetcher.add_peer(peer, inv_collector.fetcher_for_peer(peer),
                              other_last_block_index)
        block_handler.add_peer(peer)
        inv_collector.add_peer(peer)
...
def items_for_client(initial_blocks=[]):
    ...
    ap = make_add_peer(fast_forward_add_peer, blockfetcher, block_handler,
                       inv_collector, block_chain, block_store)
    ap.block_getter_task = asyncio.Task(block_getter(
        inv_q, inv_collector, block_handler, block_chain, block_store))
...
def test_BlockHandler_tcp():
    ...
    handshake_peers(peer1,
                    peer2,
                    dict(local_ip="127.0.0.1",
                         last_block_index=len(BL1)),
    ...
    def wait_for_change_q(block_chain, count):
        ...
        try:
            r = asyncio.get_event_loop().run_until_complete(asyncio.wait_for(
                done, timeout=5))
    ...
    assert [(a, b.hash(), c) for a, b, c in r] == [('add', b.hash(), idx)
                                                   for idx, b in enumerate(BL1)]
    ...
    assert [(a, b.hash(), c) for a, b, c in r] == [('add', b.hash(), idx + len(
        BL1)) for idx, b in enumerate(BL2)]
