...
def filter_size_required(element_count, false_positive_probability):
    return min(36000, int(((-1 / pow(LOG_2, 2) * element_count *
                            math.log(false_positive_probability)) + 7) // 8))
...
class BloomFilter(object):
    ...
    def add_address(self, address, address_prefix=b'\0'):
        the_hash160 = bitcoin_address_to_hash160_sec(
            address, address_prefix=address_prefix)
        self.add_item(the_hash160)

    def add_spendable(self, spendable):
        item_bytes = spendable.tx_hash + struct.pack("!L",
