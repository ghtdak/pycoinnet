...
class BitcoinPeerProtocol(asyncio.Protocol):
    ...
    def write_message(self, message_type, message_data=b''):
        ...
        logging.debug("sending message %s [%d bytes]", message_type,
                      len(packet))
    ...
    def get_msg_version_parameters_default(self):
        ...
        #  nonce (32 bit)
        #  last_block_index
        remote_address, remote_port = self.transport.get_extra_info(
            "socket").getpeername()
        return dict(version=70001,
    ...
    def start(self):
        ...
        while True:
            # schedule ping
            try:
                ping_handle = asyncio.get_event_loop().call_later(60,
                                                                  self.do_ping)
    ...
    def parse_next_message(self):
        ...
        logging.debug("message: %s (%d byte payload)", message_name,
                      len(message_data))
        ...
        if the_tuple is None:
            logging.error("unknown message: %s %s", message_name,
            ...
            if len(the_tuple) > 2:
                post_f = the_tuple[2]
            d = bitcoin_streamer.parse_as_dict(prop_names.split(), prop_struct,
                                               io.BytesIO(message_data))
            post_f(d)
    ...
    def do_ping(self):
        ...
        def pong_missing():
            logging.debug("pong missing with nonce %s, disconnecting",
    ...
    def handle_msg_version(self, version, node_type, timestamp, remote_address,
                           local_address, nonce, subversion, last_block_index,
                           when):
    ...
    def send_msg_version(self, version, subversion, node_type, current_time,
                         remote_address, remote_listen_port, local_address,
                         local_listen_port, nonce, last_block_index):
        ...
        the_bytes = bitcoin_streamer.pack_struct(
            "LQQAAQSL", version, node_type, current_time, remote, local, nonce,
            subversion, last_block_index)
    ...
    def send_msg_getdata(self, items):
        the_bytes = bitcoin_streamer.pack_struct("I" + ("v" * len(items)),
                                                 len(items), *items)
