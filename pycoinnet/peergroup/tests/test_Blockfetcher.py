...
def test_get_mined_block():
    ...
    def do_update_2(blockchain, ops):
        ...
        if not client_2_has_20_blocks_future.done() and blockchain.length(
        ) >= 20:
            client_2_has_20_blocks_future.set_result(blockchain.length())
        if not client_2_has_25_blocks_future.done() and blockchain.length(
        ) >= 25:
