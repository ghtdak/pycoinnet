...
def test_InvCollector_simple():
    ...
    f2 = asyncio.Task(run_remote_peer(peer2.new_get_next_message_f(), peer2,
                                      TX_LIST))
...
def test_InvCollector():
    ...
    for peer, txs in [(peer2, TX_LIST[:30]), (peer3, TX_LIST[30:60]), (
            peer4, TX_LIST[60:90])]:
        f = asyncio.Task(run_remote_peer(peer.new_get_next_message_f(), peer,
                                         txs))
        futures.append(f)
    ...
    f = asyncio.Task(run_local_peer(inv_collector,
...
def test_TxCollector_notfound():
    ...
    def run_local_peer(peer_list):
        ...
        while len(r) < 5:
            ...
            inv_item = yield from inv_item_q.get()
            try:
                v = yield from asyncio.wait_for(
                    inv_collector.fetch(inv_item),
                    timeout=0.5)
                if v:
                    r.append(v)
