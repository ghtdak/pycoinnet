...
class InvCollector:
    ...
    def add_peer(self, peer):
        ...
        next_message = peer.new_get_next_message_f(
            lambda name, data: name in ("inv", "notfound"))
    ...
    def fetch(self, inv_item, peer_timeout=10):
        ...
        q = asyncio.Queue()
        items = sorted(self.inv_item_db[inv_item.data].items(),
        ...
        while True:
            ...
            if timer_future in done:
                ...
                logging.debug("timeout, need to request from a new peer, %s",
                              inv_item)
                timer_future = asyncio.Task(_wait_for_timeout_and_peer(
                    q, initial_delay=peer_timeout))
            ...
            if most_recent_fetcher and most_recent_fetcher.done():
                ...
                logging.debug("got a notfound, need to try a new peer for %s",
                              inv_item)
                timer_future.cancel()
    ...
    def fetch_validate_store_item_async(self, inv_item, item_store,
                                        validator_f):
