...
class InvCollector:
            ...
            try:
                while True:
                    ...
                    for inv_item in data["items"]:
                        logging.debug("noting %s available from %s", inv_item,
                                      peer)
    ...
    def fetch(self, inv_item, peer_timeout=10):
        ...
        while True:
            ...
            q_change_future = asyncio.Task(_q_change(
                q, futures, timeout=timeout))
            all_futures = futures + [q_change_future]
            done, pending = yield from asyncio.wait(
                all_futures, return_when=asyncio.FIRST_COMPLETED)
