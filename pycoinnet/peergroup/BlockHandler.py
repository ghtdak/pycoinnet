...
class BlockHandler:
    ...
    def _watch_block_chain(self, change_q, should_download_f):
        ...
        def _download_block(block_hash, block_index):
            block = yield from self.inv_collector.fetch(InvItem(ITEM_TYPE_BLOCK,
                                                                block_hash))
            return block
        ...
        while True:
            ...
            if should_download_f(block_hash, block_index):
                ...
                self._download_task = asyncio.Task(_download_block(block_hash,
                                                                   block_index))
        ...
        def _run_handle_get(next_message):
            while True:
                ...
                if name == 'getdata':
                    ...
                    if not_found:
                        logging.debug("could not find %d blocks",
                                      len(not_found))
    ...
    def add_block(self, block):
        ...
        if the_hash not in self.block_store:
            ...
            self.inv_collector.advertise_item(InvItem(ITEM_TYPE_BLOCK,
                                                      the_hash))
    ...
    def _watch_invcollector(self, block_validator):
        while True:
            ...
            self.inv_collector.fetch_validate_store_item_async(
                inv_item, self.block_store, block_validator)
