...
class Blockfetcher:
    ...
    def add_peer(self, peer, fetcher, last_block_index):
        self.fetch_future_lookup[peer] = asyncio.Task(self.fetch_from_peer(
            peer, fetcher, last_block_index))
    ...
    def fetch_from_peer(self, peer, fetcher, last_block_index):
        ...
        while True:
            ...
            if len(pending) > 0:
                logging.debug("unfinished, decreasing loop count from %d",
                              per_loop)
                per_loop = int(per_loop * 0.5 + 1)
                done, pending = yield from asyncio.wait(futures,
                                                        timeout=loop_timeout)
            else:
                if finish_time * 3 < loop_timeout and len(
                        items_to_try) >= per_loop:
