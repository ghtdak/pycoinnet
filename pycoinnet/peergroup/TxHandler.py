...
class TxHandler:
    ...
    def add_peer(self, peer):
        ...
        def _run_mempool(next_message):
            try:
                name, data = yield from next_message()
                inv_items = [InvItem(ITEM_TYPE_TX, tx.hash())
                             for tx in self.tx_store.values()]
                logging.debug(
                    "sending inv of %d item(s) in response to mempool",
                    len(inv_items))
                if len(inv_items) > 0:
        ...
        next_getdata = peer.new_get_next_message_f(
            lambda name, data: name == 'getdata')
        peer.add_task(_run_getdata(next_getdata))
        next_mempool = peer.new_get_next_message_f(
            lambda name, data: name == 'mempool')
    ...
    def _run(self, tx_validator):
        while True:
            ...
            if inv_item.item_type != ITEM_TYPE_TX:
                continue
            self.inv_collector.fetch_validate_store_item_async(
                inv_item, self.tx_store, tx_validator)
