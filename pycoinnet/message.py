...
def fixup_merkleblock(d, _):
    ...
    if left_hash != d["header"].merkle_root:
        raise ValueError("merkle root %s does not match calculated hash %s" %
...
def _message_fixups():
    ...
    return dict(version=fixup_version,
                alert=fixup_alert,
                merkleblock=fixup_merkleblock)
...
def _make_parse_from_data():
    ...
    def init_bitcoin_streamer():
        more_parsing = [
            ("A",
             (PeerAddress.parse, lambda f, peer_addr: peer_addr.stream(f))),
