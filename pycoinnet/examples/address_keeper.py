...
class AddressKeeper:
    ...
    def __init__(self, peer, address_db):
        ...
        def get_msg_addr():
            ...
            logging.info("got %s message from %s with %d entries", name, peer,
                         len(date_address_tuples))
...
def keep_minimum_connections(event_loop, min_connection_count=4):
    ...
    while 1:
        ...
        for i in range(difference * 2):
            f = asyncio.Task(connect_to_remote(event_loop, magic_header,
                                               address_db, connections))
            tasks.add(f)
