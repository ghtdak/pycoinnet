...
class SPVClient(object):
    ...
    def __init__(self,
        ...
        self.feed_task = asyncio.Task(self.feed_merkle_blocks(
            merkle_block_index_queue))
        ...
        def create_protocol_callback():
            ...
            peer.add_task(run_peer(peer, fetcher, self.getheaders_add_peer,
...
def main():
    ...
    bloom_filter.add_item(h2b(
        "0478abb18c0c7c95348fa77eb5fd43ce963e450d797cf4878894230ca528e6c8e866c3"
        "8ad93746e04f2161a01787c82a858ee24940e9a06e41fddb3494dfe29380"))
    spendable = Spendable(0, b'', h2b_rev(
    ...
    def fetch(merkle_block_index_queue):
        while True:
            ...
            logging.info("block #%d %s with %d transactions", index,
                         merkle_block.id(), len(merkle_block.txs))
