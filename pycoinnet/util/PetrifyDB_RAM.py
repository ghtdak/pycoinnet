...
class PetrifyDB_RAM(object):
    ...
    def __init__(self, parent_to_0=ZERO_HASH):
        ...
        self.petrified_hashes_lookup = dict(
            (v, k) for k, v in enumerate(self.petrified_hashes))

    def _log(self):
        logging.debug("petrified chain is length %d",
                      len(self.petrified_hashes))
        if len(self.petrified_hashes):
            logging.debug("petrified chain starts with %s",
                          self.petrified_hashes[0])
            logging.debug("petrified chain ends with %s",
                          self.petrified_hashes[-1])
        if len(self.petrified_hashes_lookup) < len(self.petrified_hashes):
            logging.error("warning: petrified_hashes_lookup has %d members",
                          len(self.petrified_hashes_lookup))
    ...
    def item_for_index(self, idx):
        ...
        return BlockHashOnly(h=self.hash_for_index(idx),
    ...
    def add_chain(self, items):
        ...
        if items[0].previous_block_hash != last_hash:
            raise PetrifyError(
                "blockchain does not extend petrified chain (expecting %s)" %
                last_hash)
