...
class BlockChain(object):
    ...
    def __init__(self,
                 local_db,
                 petrify_db,
                 petrify_policy=default_petrify_policy):
    ...
    def _create_chain_finder(self):
        ...
        self.chain_finder.load_nodes(bh_to_node(self.local_db.item_for_hash(h))
                                     for h in self.local_db.all_hashes()
    ...
    def block_chain_size(self):
        return self.longest_local_block_chain_length(
        ) + self.petrified_block_count()

    def hash_is_known(self, h):
        return self.petrify_db.hash_is_known(h) or self.local_db.hash_is_known(
            h)
