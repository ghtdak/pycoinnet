...
class BitcoinPeerProtocol(asyncio.Protocol):
    ...
    def new_get_next_message_f(self,
                               filter_f=lambda message_name, data: True,
                               maxsize=0):

        @asyncio.coroutine
        def run(self):
            yield from asyncio.wait_for(self.connection_made_future,
                                        timeout=None)
            while True:
                try:
    ...
    def send_msg(self, message_name, **kwargs):
        ...
        logging.debug("sending message %s [%d bytes] to %s",
                      message_type.decode("utf8"), len(packet), self)
    ...
    def _parse_next_message(self):
        ...
        if blob != self.magic_header:
            raise BitcoinProtocolError("bad magic: got %s" %
                                       binascii.hexlify(blob))
        ...
        logging.debug("message %s: %s (%d byte payload)", self, message_name,
                      len(message_data))
