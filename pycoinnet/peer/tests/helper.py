...
def handshake_peers(peer1, peer2, peer_info_1={}, peer_info_2={}):
    ...
    asyncio.get_event_loop().run_until_complete(asyncio.wait([initial_handshake(
        peer1, msg1), initial_handshake(peer2, msg2)]))
    return peer1, peer2
...
def create_handshaked_peers(ip1="127.0.0.1", ip2="127.0.0.2"):
    ...
    asyncio.get_event_loop().run_until_complete(asyncio.wait([initial_handshake(
        peer1, VERSION_MSG), initial_handshake(peer2, VERSION_MSG_2)]))
    return peer1, peer2
...
def create_peers_tcp():
    ...
    server, port, future_peer = asyncio.get_event_loop().run_until_complete(
    ...
    peer2 = asyncio.get_event_loop().run_until_complete(asyncio.Task(
        run_connector(port)))
