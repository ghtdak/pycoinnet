...
def test_BitcoinPeerProtocol_send():
    ...
    assert len(DATA) == 3
    assert DATA[
        -1] == b'foodmempool\x00\x00\x00\x00\x00\x00\x00\x00\x00]\xf6\xe0\xe2'
...
def test_BitcoinPeerProtocol_multiplex():
    ...
    peer.data_received(VERSION_MSG_BIN)
    peer.data_received(VERACK_MSG_BIN)
    asyncio.get_event_loop().run_until_complete(asyncio.wait([asyncio.Task(
        async_test(nm)) for nm in next_message_list]))
    assert COUNT == 50
...
def test_queue_gc():
    ...
    tasks = [asyncio.Task(async_listen(peer.new_get_next_message_f(),
                                       delay=1)) for i in range(50)]
