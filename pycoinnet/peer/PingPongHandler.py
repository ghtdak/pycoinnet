        ...
        while True:
            ...
            if self.last_message_timestamp + self.heartbeat_rate < now:
                ...
                if nonce in self.ping_nonces:
                    # gotta hang up!
                    self.peer.stop()
            yield from asyncio.sleep(self.last_message_timestamp +
                                     self.heartbeat_rate - now)

    def handle_msg(self, peer, **kwargs):
        self.last_message_timestamp = time.time()
        logging.debug("got message, new last timestamp is %d",
                      int(self.last_message_timestamp))
